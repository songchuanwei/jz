...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <div class="formControls col-xs-1 col-sm-3"  style="margin: 0 0 0 10px;">
            <span class="select-box">
                    <select class="select" size="1" name="activity" id="activity">
                        @foreach($activitys as $activity1)
                            @if($activity1->id==$activity->id)
                                <option value="{{$activity1->id}}" selected="selected">{{$activity1->name}}</option>
                            @else
                                <option value="{{$activity1->id}}">{{$activity1->name}}</option>
                            @endif
                        @endforeach
                    </select>
            </span>
        ...
        <span class="r">共有数据：<strong>{{$activity->users()->count()}}</strong> 条</span>
    </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tbody>
            @foreach($activity->users()->paginate(10) as $user)
                <tr class="text-c">
                    <td>{{$user->id}}</td>
                    <td>{{$user->name}}</td>
                    <td class="td-manage">
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content radius">
                            ...
                            <div class="modal-body">
                                ...
                                @if(empty($user->activityPoint($activity->id)))
                                    <div class="row cl">
                                    ...
                                    <div class="row cl">
                                        <div class="row cl" style="margin: 0 0 10px 0">
                                            <label class="form-label col-xs-4 col-sm-4 text-r"><span class="c-red">*</span>加减分数：</label>
                                            <div class="formControls col-xs-8 col-sm-6">
                                                <input type="number" id="point{{$user->id}}" name="point{{$user->id}}" class="input-text" value="{{$user->activityPoint($activity->id)->point}}" placeholder="正数加 负数减">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row cl" style="margin: 0 0 10px 0">
                                        <label class="form-label col-xs-4 col-sm-4 text-r">加减分备注：</label>
                                        <div class="formControls col-xs-8 col-sm-6">
                                            <textarea cols="" rows="" class="textarea" name="content{{$user->id}}" id="content{{$user->id}}" placeholder="说点什么... (选填)">{{$user->activityPoint($activity->id)->content}}</textarea>
                                        </div>
                                    </div>
                                @endif
                                <button onclick="Check('{{$activity->id}}','{{$user->id}}');" class="btn btn-primary radius" style="margin: 0 0 10px 30px">添加</button>
                            </div>
        ...
        <div id="pull_right">
            <div class="pull-right">
                {!! $activity->users()->paginate(10)->render() !!}
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('#activity').bind('input propertychange', function() {
        $(location).attr('href', '/admin_activity/user?activity='+$("#activity").val());
    });

    function Check(activity_id,user_id) {
